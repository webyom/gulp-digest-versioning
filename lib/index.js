(function() {
  var PluginError, cp, crypto, fs, md5List, path, renamedFileMap, through;
  ...
  module.exports = function(opt) {
    ...
    return through.obj(function(file, enc, next) {
      var baseDir, content, destDir, digestLength, extnames;
      if (file.isNull()) {
      ...
      if (opt.destDir) {
        destDir = path.resolve(process.cwd(), opt.destDir);
      }
      ...
      content = content.replace(/url\(\s*([^)]+)\s*\)/mg, function(full, fileName) {
        ...
        if (/^(https?:|\/\/)/.test(fileName)) {
          return full;
        } else if (opt.skipFileName && opt.skipFileName(fileName, md5List)) {
          if (opt.fixSkipUrl) {
